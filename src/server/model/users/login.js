...
login.prototype.getUserData = function() {
  const query = {
    text:
      'SELECT company_id, customerid, empid, status, email, company_name, lname, fname, password, admin, industry, maintcode FROM login WHERE email = $1 ',
...
login.prototype.checkPassword = function(req, res, data) {
  if (data.rows[0]) {
    ...
    res
      .status(200)
      .json({ success: false, userNotify: 'That email or password is invalid' })
  }
}
