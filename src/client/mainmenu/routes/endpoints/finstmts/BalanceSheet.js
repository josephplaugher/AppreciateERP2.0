...
class BalanceSheet extends FormClass {
  constructor(props) {
    super(props);
    this.useLiveSearch = false;
    this.valRules = ValRules;
    this.route = '/stmts/balance';
    this.state = {
      dataView: false,
      userNotify: {},
      date: '',
  ...
  render() {
    const currentAssets = this.state.currentAssets.map(item => (
      <>
        <div
          className="acct-name"
          key={`${item.acctname}_${item.acctno}`}
          id={`${item.acctname}_${item.acctno}`}
        >
          {item.acctname}
        </div>
        <div className="acct-total" key={`acct-total_${item.acctno}`}>
    ...
    return (
      <>
        ...
        <div id="workingPane">
          <p className="formTitle">Balance Sheet</p>
          ...
          <EB comp="Statement Field in Income Statement">
            <div id="statementField">
              ...
              <div className="total-header">Total Current Assets</div>
              <div className="subtotal">{this.state.currentAssetsTotal}</div>

              <div className="header">Fixed Assets</div>
              <div className="statement-line">{fixedAssets}</div>
              <div className="total-header">Total Fixed Assets</div>
              <div className="subtotal">{this.state.fixedAssetsTotal}</div>

              <div className="total-header">Total Assets</div>
              <div className="subtotal">{this.state.assetsTotal}</div>

              <div className="header">Current Liabilities</div>
              <div className="statement-line">{currentLiabilities}</div>
              <div className="total-header">Total Current Liabilities</div>
              <div className="subtotal">
                {this.state.currentLiabilitiesTotal}
              </div>

              <div className="header">Long-Term Liabilities</div>
              <div className="statement-line">{longTermLiabilities}</div>
              <div className="total-header">Total Long-Term Liabilities</div>
              <div className="subtotal">
                {this.state.longTermLiabilitiesTotal}
              </div>

              <div className="total-header">Total Liabilities</div>
              <div className="subtotal">{this.state.liabilitiesTotal}</div>

              <div className="header">Equity</div>
              <div className="statement-line">{equity}</div>
              <div className="total-header">Total Equity</div>
              <div className="subtotal">{this.state.equityTotal}</div>
            </div>
          </EB>
